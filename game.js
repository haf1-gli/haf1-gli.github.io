// game.js - Main game loop with multiplayer synchronization (BUGS FIXED)
...
class GameState {
    constructor() {
        ...
        this.gameStarted = false; // FIX: Track if game actually started
    }
}
...
function createExplosion(position, isLocal = true) {
    ...
    const explosionMaterial = new THREE.MeshBasicMaterial({
        map: explosionSpriteSheet.clone(), // FIX: Clone texture
        transparent: true,
    ...
    // FIX: Use PositionalAudio properly with distance calculation
    const sound = new THREE.PositionalAudio(audioListener);
    sound.setBuffer(explosionSoundBuffer);
    sound.setRefDistance(20); // Increased from 10
    sound.setRolloffFactor(1);
    sound.setMaxDistance(200); // Increased from 100
    sound.setVolume(1.0);
    
    // FIX: Don't attach to explosion mesh, use scene position
    ...
    // Damage detection (only check if local bullet)
    if (isLocal) {
...
function checkExplosionDamage(position) {
    // FIX: Check both tanks for damage
    // Check player 1 tank
    if (player1Tank) {
    ...
    // FIX: Check collidables (trees) and sync destruction
    for (let i = collidables.length - 1; i >= 0; i--) {
        ...
        if (collidable.userData.type === 'tree') {
            ...
            if (distance < explosionBlastRadius + 5) {
                ...
                // FIX: Send tree destruction to network
                if (gameState.isMultiplayer && !gameState.isTestMode) {
                    webrtcManager.send('TREE_DESTROYED', {
...
function handleRemoteDamage(data) {
    // FIX: Apply damage to correct player
...
function sendPlayerUpdate() {
    ...
    // FIX: Include Y position in update
...
function updateRemotePlayer(data) {
    ...
    // FIX: Use full position data including Y
...
function interpolateRemotePlayer(delta) {
    ...
    // FIX: Interpolate position (including Y)
...
const playerUpdateInterval = 1 / 20; // 20Hz
...
function init() {
    ...
    // Generate map (host only generates, then syncs)
    generateMap();
...
function generateMap() {
    ...
    // Load decorations (host will generate and sync positions)
    ...
    gltfLoader.load(
        ...
        (gltf) => {
            ...
            for (let i = 0; i < count; i++) {
                ...
                // FIX: Raycast to get ground Y position
    ...
    gltfLoader.load(
        ...
        (gltf) => {
            ...
            for (let i = 0; i < count; i++) {
                ...
                tree.userData.id = `tree_${i}`; // FIX: Add ID for sync
...
function setupWebRTCListeners() {
    webrtcManager.on('connected', (role) => {
        ...
        // FIX: Host sends game mode AND map data
        if (role === ROLES.HOST) {
...
function setupCutsceneListeners() {
    cutsceneManager.on('trigger-explosion', (position) => {
        createExplosion(position, false); // Don't check damage during cutscene
    });
}
...
function handleNetworkMessage(message) {
    switch (message.type) {
        ...
        case MSG_TYPES.GAME_START:
            ...
            // FIX: Receive map data from host
...
// FIX: Handle tree destruction from remote
...
function onKeyDown(event) {
    if (gameState.isControlsLocked || !gameState.gameStarted) return; // FIX: Check game started
...
function checkExplosionDamage(position) {
    // Check local player
    const localTank = gameState.localPlayer.tank;
    if (localTank) {
        const distance = position.distanceTo(localTank.position);
        if (distance < explosionBlastRadius) {
            const damageRatio = Math.max(0, 1.0 - (distance / explosionBlastRadius));
            const damage = explosionMaxDamage * damageRatio;
            
            gameState.localPlayer.health -= damage;
            gameState.localPlayer.health = Math.max(0, gameState.localPlayer.health);
    ...
    // Check remote player
    const remoteTank = gameState.remotePlayer.tank;
    if (remoteTank) {
        const distance = position.distanceTo(remoteTank.position);
        if (distance < explosionBlastRadius) {
            const damageRatio = Math.max(0, 1.0 - (distance / explosionBlastRadius));
            const damage = explosionMaxDamage * damageRatio;
            ...
            if (gameState.isMultiplayer && !gameState.isTestMode) {
                webrtcManager.send(MSG_TYPES.DAMAGE, { damage });
            }
        }
    }
    
    // Check collidables (trees)
    for (let i = collidables.length - 1; i >= 0; i--) {
        const collidable = collidables[i];
        if (collidable.userData.type === 'tree') {
            const distance = position.distanceTo(collidable.position);
...
// === DAMAGE HANDLING ===
function handleRemoteDamage(data) {
    gameState.localPlayer.health -= data.damage;
    gameState.localPlayer.health = Math.max(0, gameState.localPlayer.health);
    updateHealthBar();
...
function updateDummyAI(delta) {
    ...
    // Simple AI: move in circles and occasionally shoot
    const tank = gameState.remotePlayer.tank;
...
function animate() {
    ...
    // Update local player
    if (gameState.localPlayer && gameState.localPlayer.tank && !gameState.isControlsLocked) {
        updateLocalPlayer(delta);
    }
    
    // Update remote player
    if (gameState.remotePlayer && gameState.remotePlayer.tank) {
        if (gameState.isTestMode) {
            updateDummyAI(delta);
        } else {
            interpolateRemotePlayer(delta);
